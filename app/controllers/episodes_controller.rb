class EpisodesController < ApplicationController
  before_action :authenticate_podcast!, exept: [:show]
  ...
  def update
    if @episode.update episode_params
      redirect_to podcast_episode_path(@podcast, @episode), notice: "Episode succesfully updated!"
    else
      render "edit"
    end
  end
  ...
  def episode_params
    params.require(:episode).permit(:title, :description, :episode_logo, :video)
  end
